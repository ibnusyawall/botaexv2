...
const HandlerMessageBot = require('./handler')
const ANIMHANDLER = require('./handler/animHandler')
const CUSTOMHANDLER = require('./handler/customHandler')
const TPROPERTIESHANDLER = require('./handler/toolspropertiesHandler')
const TPARAM1HANDLER = require('./handler/toolsparam1Handler')
const DOWNLOADERHANDLER = require('./handler/downloaderHandler')
...
client.on('message-new', async (m) => {
    ...
    switch (isUser) {
        case true:
            ...
            /*(count === ran) ? await updateXPUser(id, ran) : false

            (count === ran) ? await client.sendMessage(from, `yeay! kamu mendapatkan *${ran}* xp!`, text) : false
            */
            var loop;
            ...
            function start() {
                setTimeout( async () => {
                    ...
                    if (usercmd) {
                        /*(count === count.length) ? await updateXPUser(id, count.length) : ''
                        (count === count.length) ? await client.sendMessage(from, `yeay! kamu mendapatkan *${count.length}* xp!`, text) : false
                        (count === count.length) ? await deleteCmdUser(__ids) : false*/
                        if (count === count.length) return client.sendMessage(from, `yeay! kamu mendapatkan *${count.length}* xp!`, text)

                    }
                }, 30000)
            }

            //var int = [5000, 10000, 30000, 20000, 15000]
            //Object.keys(vp).indexOf(id) >= 0 ? vp[`${id}`].cmd.push(args.join(' ')) : vp = { ids: `${id}`, cmd: [args.join(' ')]}

            //vp.push(`${id}`: {ids: id, command: [args.join(' ')]})
            //var obj = {ids: id, command: [args.join(' ')]}; vp.push(obj[`${id}`])
...
//                client2.reply(from, String(ran))
    ...
    /*if (m.message.extendedTextMessage.text == 'huntu') {
        if (m.key.remoteJid.indexOf('@s.whatsapp.net') > 0) {
            var options = {
                contextInfo: {
                    participant: '6285157711053@s.whatsapp.net',
                    quotedMessage: {
                        extendedTextMessage: {
                            text: '𝗔𝗘𝗫-𝗕𝗢𝗧'
                        }
            ...
            client.sendMessage(m.key.remoteJid, `Terima kasih telah menghubungi Ibnusyawall. Silakan beri tahu apa yang dapat kami bantu.`, MessageType.text, options)
        }
    }*/
...
//    CUSTOMHANDLER(client, m)
//    TPROPERTIESHANDLER(client, m)
//    ANIMHANDLER(client, m)
//    TPARAM1HANDLER(client, m)
//    DOWNLOADERHANDLER(client, m)
